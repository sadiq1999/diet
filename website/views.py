...
def plan():
    ...
    # fist day in current month
    start_day = today - datetime.timedelta(days=today.day - 1)
    temp_start_day = start_day

    if request.method == 'POST':

        num_month1 = days_in_current_month  # For Ex
        YourPlan()
        ...
        while num_month1 > 0:
            ...
            db.session.commit()
            temp_start_day += datetime.timedelta(days=1)
            num_month1 -= 1
...
def exercises():
    ...
    images = os.path.join(SITE_ROOT, "static", "exercises.json")
    with open(images) as d:
        data = json.load(d)

    name1 = []

    for exercise in data['exercises']:
        name1.append(exercise['name'])

    return render_template('exercises.html', user=current_user, name1=name1)
